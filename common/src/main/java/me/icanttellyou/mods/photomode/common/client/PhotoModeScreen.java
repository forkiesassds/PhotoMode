...
public class PhotoModeScreen extends Screen {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (isTakingScreenshot) {
            ...
            if (showInfoText) {
                drawCenteredTextWithShadow(matrices, textRenderer, new TranslatableText("gui.photomode.helpText").asOrderedText(), width / 2, height - 56, 0xFFFFFF);
            }
        }
        updateGui();
    }
    ...
    private void initWidgets() {
        addDrawableChild(centerScreen = new ButtonWidget(width - 150, 0, 150, 20, new TranslatableText("gui.photomode.centerCamera"), (button) -> {
            cameraPanXGoal = 0.0F;
            cameraPanYGoal = 0.0F;
            cameraRotationGoal = 0.0F;
        }));

        addDrawableChild(showPlayer = new ButtonWidget(width - 150, 0, 150, 20, new TranslatableText("gui.photomode.showPlayer", ScreenTexts.ON), (button) ->
        ...
        addDrawableChild(shader = new ButtonWidget(width - 150, 0, 150, 20, new TranslatableText("gui.photomode.shader", new TranslatableText("gui.photomode.none")), (button) -> {
            cycleShader();
            this.intensitySlider.active = client.gameRenderer.getShader() != null;
        }));

        intensitySlider = new PhotoModeSliderWidget(width - 150, 0, 150, 20, new TranslatableText("gui.photomode.intensity"), shaderIntensity);
    ...
    private void updateGui() {
        timeSlider.setText(new TranslatableText("gui.photomode.time", timeSlider.value == 0.0f ? new TranslatableText("gui.photomode.default") : (long)(timeSlider.value * 24000.0f)));
        fogSlider.setText(new TranslatableText("gui.photomode.fog", (int)(fogSlider.value * 100.0f)));
        tiltSlider.setText(new TranslatableText("gui.photomode.tilt", (int)(tiltSlider.value * 90.0f) == 30 ? new TranslatableText("gui.photomode.default") : (int)(tiltSlider.value * 90.0f)).append(Text.of(" ")).append((int)(tiltSlider.value * 90.0f) == 30 ? Text.of("") : new TranslatableText("gui.photomode.degrees")));
        ...
        Text shaderName = new TranslatableText("gui.photomode.none");
        if (postEffectProcessor != null) {
            String[] splitPath = postEffectProcessor.getName().split("/");

            String name = splitPath[splitPath.length - 1];
            name = name.substring(0, name.indexOf("."));
            shaderName = new TranslatableText("photomode.shader." + name);
        }

        shader.setMessage(new TranslatableText("gui.photomode.shader", shaderName));
        intensitySlider.setText(new TranslatableText("gui.photomode.intensity", (int)(intensitySlider.value * 100.0F)));
